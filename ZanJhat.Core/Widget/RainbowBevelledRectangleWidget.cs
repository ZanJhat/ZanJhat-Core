...
{
    public class RainbowBevelledRectangleWidget : Widget
    {
        ...
        public float RainbowDensity { get; set; }

        public bool ProportionalRainbow { get; set; }

        public float HueSpeed { get; set; }

        public RainbowBevelledRectangleWidget()
        {
        ...
        public override void Draw(DrawContext dc)
        {
            ...
            // Tránh việc CenterColor trùng cờ Alpha của Bevel
            if (centerColor.A == 254) centerColor.A = 255;

            // MAGIC ALPHA TRICK: Gán Alpha = 254 và Màu Trắng để phân biệt Bevel Vertex trong Cache và giữ lại thông số ánh sáng.
            Color bevelRenderColor = OutlineRainbowColor ? new Color((byte)255, (byte)255, (byte)255, (byte)254) : BevelColor;
            ...
            if (Texture != null)
            {
                ...
                if (flag)
                {
                    if (m_flatBatch == null) m_flatBatch = dc.PrimitivesRenderer2D.FlatBatch(0, DepthStencilState.None);
                ...
                if (m_texturedBatch == null)
                {
                    m_texturedBatch = dc.PrimitivesRenderer2D.TexturedBatch(
                        Texture, false, 1, null, null, null, TextureLinearFilter ? SamplerState.LinearWrap : SamplerState.PointWrap
                    );
                }

                int textStart = m_texturedBatch.TriangleVertices.Count;
                m_texturedBatch.QueueBatch(m_cachedTexturedBatch, GlobalTransform, GlobalColorTransform);
                int textEnd = m_texturedBatch.TriangleVertices.Count;

                if (OutlineRainbowColor)
                {
                    ApplyRainbowToTextured(m_texturedBatch, m_cachedTexturedBatch, textStart, textEnd);
                }
                return;
            }

            // Path xử lý khối không có Texture (Flat)
            if (!m_cachedBatchesValid)
            {
            ...
            int flatStart = m_flatBatch.TriangleVertices.Count;
            m_flatBatch.QueueBatch(m_cachedFlatBatch, GlobalTransform, GlobalColorTransform);
            int flatEnd = m_flatBatch.TriangleVertices.Count;

            if (OutlineRainbowColor)
            {
                ApplyRainbowToFlat(m_flatBatch, m_cachedFlatBatch, flatStart, flatEnd);
            }
        }

        // ============================================
        // Rainbow Vertex Processors
        // ============================================

        private void ApplyRainbowToFlat(FlatBatch2D batch, FlatBatch2D cachedBatch, int startIdx, int endIdx)
        {
            float time = (float)Time.RealTime;
            Vector2 globalCenter = Vector2.Transform(ActualSize / 2f, GlobalTransform);
            float perimeter = (ActualSize.X + ActualSize.Y) * 2f;
            if (perimeter <= 0f) perimeter = 1f;

            for (int i = startIdx; i < endIdx; i++)
            {
                int cachedIndex = i - startIdx;
                var cachedV = cachedBatch.TriangleVertices.Array[cachedIndex];

                // Chỉ xử lý các đỉnh Bevel thông qua Magic Alpha
                if (cachedV.Color.A == 254)
                {
                    var v = batch.TriangleVertices.Array[i];

                    // Tính toán góc xoay từ tâm widget để tạo Gradient ôm vòng mượt mà
                    Vector2 localDir = v.Position.XY - globalCenter;

                    float angle = MathF.Atan2(localDir.Y, localDir.X);
                    float angleT = (angle + MathF.PI) / (MathF.PI * 2f);

                    float hueOffset = ProportionalRainbow ? (angleT * RainbowDensity) : ((angleT * perimeter / 100f) * RainbowDensity);
                    Color rainbow = ColorUtils.ColorFromHSV(time * HueSpeed + hueOffset);

                    // Lấy lại độ sáng (Lighting) đã được nướng vào kênh Đỏ (Red) của viền trắng lúc Cache
                    float lightFactor = cachedV.Color.R / 255f;

                    batch.TriangleVertices.Array[i].Color = new Color(
                        (byte)(rainbow.R * lightFactor),
                        (byte)(rainbow.G * lightFactor),
                        (byte)(rainbow.B * lightFactor),
                        v.Color.A // Giữ nguyên mức Alpha gốc 
                    );
                }
            }
        }

        private void ApplyRainbowToTextured(TexturedBatch2D batch, TexturedBatch2D cachedBatch, int startIdx, int endIdx)
        {
            float time = (float)Time.RealTime;
            Vector2 globalCenter = Vector2.Transform(ActualSize / 2f, GlobalTransform);
            float perimeter = (ActualSize.X + ActualSize.Y) * 2f;
