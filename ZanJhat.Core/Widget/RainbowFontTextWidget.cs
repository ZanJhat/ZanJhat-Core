...
{
    public class RainbowFontTextWidget : Widget
    {
        ...
        public override void Draw(DrawContext dc)
        {
            if (!string.IsNullOrEmpty(Text) && Color.A != 0)
            {
                ...
                Color baseColor = Color * GlobalColorTransform;
                ...
                foreach (string line in m_lines)
                {
                    ...
                    // 1. Vẽ DropShadow (màu đen) nếu có. Bỏ qua hiệu ứng cầu vồng cho phần bóng đổ.
                    if (DropShadow)
                    {
                        fontBatch2D.QueueText(
                            ...
                            0f,
                            new Color((byte)0, (byte)0, (byte)0, baseColor.A),
                            anchor,
                    ...
                    // Lưu lại số lượng vertex hiện tại TRƯỚC KHI vẽ text chính
                    int mainTextStartCount = fontBatch2D.TriangleVertices.Count;

                    // 2. Vẽ Main Text (màu base mặc định)
                    fontBatch2D.QueueText(
                        line,
                        vector,
                        0f,
                    ...
                    // 3. Can thiệp áp dụng hiệu ứng Rainbow trực tiếp lên các vertex vừa sinh ra
                    if (RainbowColor)
                    {
                        int mainTextEndCount = fontBatch2D.TriangleVertices.Count;
                        float referenceSize = (TextOrientation == TextOrientation.Horizontal) ? ActualSize.X : ActualSize.Y;

                        // Tránh lỗi chia cho 0 nếu ActualSize bị bằng 0
                        if (referenceSize <= 0f) referenceSize = 1f;

                        for (int i = mainTextStartCount; i < mainTextEndCount; i++)
                        {
                            // Lấy toạ độ tĩnh của vertex tuỳ thuộc vào hướng Text
                            float pos = (TextOrientation == TextOrientation.Horizontal)
                                ? fontBatch2D.TriangleVertices.Array[i].Position.X
                                : fontBatch2D.TriangleVertices.Array[i].Position.Y;

                            float hueOffset;
                            if (ProportionalRainbow)
                            {
                                hueOffset = (pos / referenceSize) * RainbowDensity;
                            }
                            else
                            {
                                hueOffset = (pos / 100f) * RainbowDensity;
                            }

                            // Tính màu Rainbow
                            Color rainbowColor = ColorUtils.ColorFromHSV(time * HueSpeed + hueOffset) * GlobalColorTransform;

                            // Giữ lại giá trị Alpha ban đầu (quan trọng để font không bị viền đen hoặc mờ lỗi)
                            rainbowColor.A = fontBatch2D.TriangleVertices.Array[i].Color.A;

                            // Ghi đè màu mới vào mảng Vertex
                            fontBatch2D.TriangleVertices.Array[i].Color = rainbowColor;
                        }
                    }
